...
(() => {
  ...
  const AI_NAME = IN_CLAUDE ? 'Claude' : 'Gemini';
  const MAX_CAMS = 8;
    ...
    return (
      {
        ...
        rate_limited: IN_CLAUDE ? 'Llegaste al límite de uso por ahora. Espera unos minutos y vuelve a intentarlo.' : 'Llegaste al límite gratuito de Gemini. Espera un minuto; si es el límite diario, vuelve mañana.',
        ...
        refused: `${AI_NAME} no pudo analizar este contenido.`,
        ...
        empty_completion: `${AI_NAME} no devolvió texto. Vuelve a intentarlo con una indicación más simple.`,
        bad_key: 'Google rechazó la clave de Gemini. Vuelve a pegarla en Preparación del video.',
        bad_model: 'El modelo de Gemini ya no está disponible. Olvida la clave y vuelve a guardarla para elegir uno nuevo.',
      }[c] || `Hubo un problema de conexión con ${AI_NAME}. Vuelve a intentarlo.`
    );
  }
  ...
  function aiReady(needImages = true) {
    ...
    if (!S.sample) {
      toast(IN_CLAUDE ? 'El análisis con Claude solo funciona con la página abierta dentro de claude.ai.' : 'Pega tu clave gratuita de Gemini en “Preparación del video” para que la IA describa las cámaras.', 'err');
      return false;
    }
    if (S.aiOff) {
      toast(S.aiOff, 'err');
  ...
  // ---------------------------------------------------------------------------
  // Modo archivo local (fuera de claude.ai): lector de texto en el computador,
  // Gemini con la clave gratuita del usuario, mapa OpenStreetMap y descargas.
  // ---------------------------------------------------------------------------
  const TESS_URL = 'https://cdn.jsdelivr.net/npm/tesseract.js@5.1.1/dist/tesseract.min.js';
  const OCR_FULL = { text: true, blocks: true, hocr: false, tsv: false };
  const OCR_TEXT = { text: true, blocks: false, hocr: false, tsv: false };
  let ocrWorkerP = null;
    ...
    if (!ocrWorkerP) {
      ocrWorkerP = (async () => {
        ...
        try {
          ...
          w = await window.Tesseract.createWorker('eng', 1, { cacheMethod: 'none' });
        }
        await w.setParameters({ tessedit_char_whitelist: '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ.:/-$ ', tessedit_pageseg_mode: '6' });
        return w;
      })().catch(() => {
        ocrWorkerP = null;
        throw { code: 'local', message: 'No se pudo cargar el lector de texto. Revisa tu conexión a internet: se descarga solo la primera vez.' };
      });
    }
    return ocrWorkerP;
  }
  // Maneras de preparar el texto, de la más a la menos agresiva. `k` es el aumento para un video
  // de 440 px de alto y `dil` cuánto se engrosa el trazo.
  const OCR_VARIANTS = [
    { k: 3, dil: 2 },
    { k: 2, dil: 1 },
    { k: 3, dil: 1 },
  ...
  function ocrVariant(frame, v) {
    const scale = clamp((v.k * 440) / frame.height, 0.75, 4);
    return { scale, dil: Math.round((v.dil * frame.height * scale) / (v.k * 440)) };
  }
  // El texto del equipo es blanco y muchas veces solo el contorno de cada letra: se aísla lo blanco,
  // se engrosa el trazo para cerrar las letras huecas y queda negro sobre fondo blanco.
  function ocrPrep(src, sx, sy, sw, sh, v) {
    const pad = 16;
    const W = Math.round(sw * v.scale) + pad * 2;
    const H = Math.round(sh * v.scale) + pad * 2;
    const c = canvas(W, H);
    const g = c.getContext('2d');
    g.fillStyle = '#000';
    g.fillRect(0, 0, W, H);
    g.imageSmoothingQuality = 'high';
    g.drawImage(src, sx, sy, sw, sh, pad, pad, W - pad * 2, H - pad * 2);
    const d = g.getImageData(0, 0, W, H);
    const a = d.data;
    let ink = new Uint8Array(W * H);
    for (let p = 0, i = 0; p < ink.length; p++, i += 4) {
      const mn = Math.min(a[i], a[i + 1], a[i + 2]);
      const mx = Math.max(a[i], a[i + 1], a[i + 2]);
      ink[p] = mn - (mx - mn) * 1.5 >= 168 ? 1 : 0;
    }
    const r = v.dil;
    if (r > 0) {
      ...
      for (let y = 0; y < H; y++) {
        ...
        for (let x = 0; x < W; x++) {
          ...
          let on = 0;
          for (let k = Math.max(0, x - r); k <= x1 && !on; k++) on = ink[row + k];
          tmp[row + x] = on;
        }
      }
      ...
      for (let y = 0; y < H; y++) {
        ...
        for (let x = 0; x < W; x++) {
          let on = 0;
          for (let k = Math.max(0, y - r); k <= y1 && !on; k++) on = tmp[k * W + x];
          out[y * W + x] = on;
        }
      }
      ink = out;
    }
    for (let p = 0, i = 0; p < ink.length; p++, i += 4) a[i] = a[i + 1] = a[i + 2] = ink[p] ? 0 : 255;
    g.putImageData(d, 0, 0);
    return { canvas: c, pad, scale: v.scale };
  }
  async function ocrRegion(w, frame, box, v, full) {
    const sx = Math.max(0, Math.floor(box.left * frame.width));
    const sy = Math.max(0, Math.floor(box.top * frame.height));
    const sw = Math.max(8, Math.min(frame.width - sx, Math.ceil((box.right - box.left) * frame.width)));
    const sh = Math.max(6, Math.min(frame.height - sy, Math.ceil((box.bottom - box.top) * frame.height)));
    const pp = ocrPrep(frame, sx, sy, sw, sh, ocrVariant(frame, v));
    const { data } = await w.recognize(pp.canvas, {}, full ? OCR_FULL : OCR_TEXT);
    const toFrame = (bb) => ({
      arriba: (sy + (bb.y0 - pp.pad) / pp.scale) / frame.height,
      abajo: (sy + (bb.y1 - pp.pad) / pp.scale) / frame.height,
      izquierda: (sx + (bb.x0 - pp.pad) / pp.scale) / frame.width,
      derecha: (sx + (bb.x1 - pp.pad) / pp.scale) / frame.width,
    });
    return { text: data.text || '', lines: data.lines || [], toFrame };
  }

  // Letras que el lector confunde con números en la tipografía de los equipos, y lecturas
  // equivocadas de la letra del hemisferio (S leída como 5, W como K, etc.).
  const DIGITISH = { O: '0', D: '0', Q: '0', U: '0', I: '1', L: '1', J: '1', T: '7', Z: '2', B: '8', G: '6', A: '4' };
  const HEMI = { S: 'S', 5: 'S', 8: 'S', $: 'S', N: 'N', H: 'N', W: 'W', K: 'W', V: 'W', M: 'W', X: 'W', Y: 'W', E: 'E', F: 'E', O: 'W', 0: 'W' };
  function osdText(raw) {
    return String(raw || '')
      .toUpperCase()
      .replace(/,/g, '.')
      .replace(/\s*\.\s*/g, '.')
      .replace(/[A-Z0-9.$]+/g, (tok) => {
        if ((tok.match(/\d/g) || []).length < 4) return tok;
        const tail = /[\d.][A-Z$]$/.test(tok) ? tok.slice(-1) : '';
        const body = tail ? tok.slice(0, -1) : tok;
        return body.replace(/[A-Z]/g, (ch) => DIGITISH[ch] || ch) + tail;
      })
      .replace(/\s+/g, ' ');
  }
  // Si el texto no deja ver la letra del hemisferio, se usa la zona horaria del computador.
  const TZ_SIGNS = (() => {
    let tz = '';
    try {
    ...
    const south = /^(America\/(Santiago|Punta_Arenas|Argentina|Buenos_Aires|Cordoba|Mendoza|Sao_Paulo|Rio_Branco|Porto_Velho|Manaus|Cuiaba|Campo_Grande|Belem|Fortaleza|Recife|Maceio|Bahia|Araguaina|Santarem|Noronha|Lima|La_Paz|Asuncion|Montevideo|Guayaquil)|Australia\/|Antarctica\/|Pacific\/(Auckland|Chatham|Fiji|Tongatapu|Easter)|Africa\/(Johannesburg|Maputo|Harare|Lusaka|Windhoek|Gaborone|Luanda|Lubumbashi|Dar_es_Salaam|Blantyre)|Indian\/(Mauritius|Reunion|Antananarivo))/.test(tz);
    return { lat: south ? -1 : 1, lon: new Date().getTimezoneOffset() > 0 ? -1 : 1 };
  })();
  // Coordenadas en los formatos habituales de los MDVR:
  // 33.25.30640S (grados.minutos.decimales), 33°25'18.4"S, -33.421773 o S33.421773.
  function parseCoordsText(raw, signs) {
    const t = osdText(raw);
    const vals = [];
    const push = (value, letter) => vals.push({ value, letter: HEMI[letter] || null });
    for (const m of t.matchAll(/([NSEWO])?(\d{1,3})\.(\d{1,2})\.(\d{2,7})([A-Z$]?)/g)) {
      let frac = m[4];
      let letter = m[5] || m[1] || '';
      if (frac.length > 5) {
        if (!letter) letter = frac[5];
        frac = frac.slice(0, 5);
      }
      const min = +`${m[3]}.${frac}`;
      if (min < 60) push(+m[2] + min / 60, letter);
    }
    if (vals.length < 2) {
      vals.length = 0;
      for (const m of t.matchAll(/(\d{1,3})\s?[°º*]\s?(\d{1,2})\s?['’]\s?(\d{1,2}(?:\.\d+)?)\s?["”]?\s?([NSEWO])?/g)) {
        if (+m[2] < 60 && +m[3] < 60) push(+m[1] + +m[2] / 60 + +m[3] / 3600, m[4]);
      }
    }
    if (vals.length < 2) {
      vals.length = 0;
      for (const m of t.matchAll(/([NSEWO])?\s?(-?\d{1,3}\.\d{4,})\s?([NSEWO])?/g)) push(parseFloat(m[2]), m[3] || m[1]);
    }
    if (vals.length < 2) return null;
    let lat = vals.find((v) => v.letter === 'N' || v.letter === 'S');
    let lon = vals.find((v) => v.letter === 'E' || v.letter === 'W');
    if (!lat || !lon || lat === lon) {
      lat = vals[0];
      lon = vals[1];
    }
    const latL = lat.letter === 'N' || lat.letter === 'S';
    const lonL = lon.letter === 'E' || lon.letter === 'W';
    let la = Math.abs(lat.value);
    let lo = Math.abs(lon.value);
    if (latL) la *= lat.letter === 'S' ? -1 : 1;
    else la = lat.value < 0 ? lat.value : la * ((signs && signs.lat) || TZ_SIGNS.lat);
    if (lonL) lo *= lon.letter === 'W' ? -1 : 1;
    else lo = lon.value < 0 ? lon.value : lo * ((signs && signs.lon) || TZ_SIGNS.lon);
    return validLL(la, lo) ? { lat: la, lon: lo, latL, lonL } : null;
  }
  function parseClockText(raw) {
    const t = osdText(raw);
    let m = t.match(/(\d{2})[/.-](\d{2})[/.-](\d{4})\D{1,4}(\d{2})\D?(\d{2})\D?(\d{2})/);
    if (m) return mk(+m[3], +m[2], +m[1], +m[4], +m[5], +m[6]);
    m = t.match(/(\d{4})[/.-](\d{2})[/.-](\d{2})\D{1,4}(\d{2})\D?(\d{2})\D?(\d{2})/);
    if (m) return mk(+m[1], +m[2], +m[3], +m[4], +m[5], +m[6]);
    return null;
  }
  function parseSpeedText(raw) {
    const m = String(raw || '').toUpperCase().match(/(\d{1,3})\s?K\s?[MNH]/);
    return m ? +m[1] : null;
  }
  const parseCanal = (raw) => {
    const m = String(raw || '').toUpperCase().match(/\bC[HN]\s?0?([0-9TILZS]{1,2})\b/);
    if (!m) return null;
    const n = +m[1].replace(/[A-Z]/g, (ch) => ({ T: '7', I: '1', L: '1', Z: '2', S: '5' })[ch]);
    return n >= 1 && n <= 64 ? n : null;
  };
  const parseBus = (raw) => {
    const m = String(raw || '').toUpperCase().match(/\b(?!CH\d)([A-Z]{2,5}-\d{1,4})\b/);
    return m ? m[1] : null;
  };

  // Franjas donde los equipos suelen escribir los datos: arriba y abajo de la imagen.
  const OSD_STRIPS = [
    { top: 0, bottom: 0.2, left: 0, right: 1 },
    { top: 0.8, bottom: 1, left: 0, right: 1 },
  ];
  const CH_BOXES = [
    { top: 0, bottom: 0.25, left: 0, right: 0.3 },
    { top: 0.75, bottom: 1, left: 0, right: 0.3 },
  ];
  async function readStrips(w, frame, v, strips = OSD_STRIPS) {
    const found = { coords: null, clock: null, speed: null, canal: null, bus: null, boxes: [], texto: [] };
    for (const strip of strips) {
      const r = await ocrRegion(w, frame, strip, v, true);
      for (const l of r.lines) {
        const txt = (l.text || '').trim();
        if (!txt) continue;
        found.texto.push(txt);
        const c = parseCoordsText(txt);
        if (c && !found.coords) {
          found.coords = c;
          found.boxes.push(r.toFrame(l.bbox));
        }
        const k = parseClockText(txt);
        if (k != null && found.clock == null) {
          found.clock = k;
          found.boxes.push(r.toFrame(l.bbox));
        }
        if (found.speed == null) found.speed = parseSpeedText(txt);
        if (found.canal == null) found.canal = parseCanal(txt);
        if (!found.bus) found.bus = parseBus(txt);
      }
    }
    return found;
  }

  async function detectLocal(b) {
    busySet('Preparando el lector de texto (solo la primera vez tarda un poco)…', null);
    const w = await ocrWorker();
    const cams = S.cams.filter((c) => !c.error && isFinite(c.duration));
    const items = [];
    let best = null;
    for (const cam of cams) {
      busySet(`Leyendo el texto en pantalla de ${cam.label}…`, null);
      const t = clamp(cam.duration * 0.1, 0, Math.max(0, cam.duration - 1));
      const frame = await grabAt(cam, t);
      if (b.stopped) return null;
      S.frameCache.set(cam.key, frame);
      // Mientras no se encuentren coordenadas se prueban todas las preparaciones; después basta
      // con la que funcionó para reconocer el canal de las demás cámaras.
      const order = best ? [best.ocr] : OCR_VARIANTS.map((_, i) => i);
      let f = null;
      let vi = order[0];
      for (const i of order) {
        const r = await readStrips(w, frame, OCR_VARIANTS[i]);
        if (b.stopped) return null;
        if (!f || (r.coords && !f.coords) || (!f.coords && r.clock != null && f.clock == null)) {
          f = r;
          vi = i;
        }
        if (r.coords) break;
      }
      // El canal (CH7) va en un rótulo corto en una esquina: se lee mejor aislado.
      for (let i = 0; i < OCR_VARIANTS.length && f.canal == null; i++) {
        const r = await readStrips(w, frame, OCR_VARIANTS[i], CH_BOXES);
        if (b.stopped) return null;
        f.canal = r.canal;
        f.bus = f.bus || r.bus;
      }
      ...
      const item = {
        key: cam.key,
        canal: f.canal != null ? `CH${f.canal}` : null,
        bus: f.bus,
        texto: f.texto.join(' | ').slice(0, 300),
        has: !!coords,
        bands: normBands(f.boxes),
        lat: coords ? coords.lat : null,
        lon: coords ? coords.lon : null,
        signs: coords ? { lat: coords.latL ? Math.sign(coords.lat) : 0, lon: coords.lonL ? Math.sign(coords.lon) : 0 } : null,
        clock: f.clock,
        speed: f.speed,
        t: t + cam.offset,
        ocr: vi,
      ...
      items.push(item);
      if (coords && !best) best = item;
      if (f.canal != null && /^CÁM /.test(cam.label)) {
        cam.label = `CH${f.canal}`;
        if (cam.lblInput) cam.lblInput.value = cam.label;
      }
      if (f.bus && !S.busId) S.busId = f.bus;
      if (f.clock != null && !S.osdClockRef) S.osdClockRef = { t: t + cam.offset, clock: f.clock };
    }
    S.osd = { items, camKey: best ? best.key : S.osd.camKey || (items[0] && items[0].key) || null, bands: {} };
    persist();
    return best;
  }

  async function extractLocal(b) {
    ...
    busySet('Preparando el lector de texto…', null);
    const w = await ocrWorker();
    // Se parte con la preparación que funcionó al detectar; si una lectura falla se prueba la
    // siguiente y, si esa acierta, pasa a ser la principal.
    let order = OCR_VARIANTS.map((_, i) => i);
    const first = item && Number.isInteger(item.ocr) ? item.ocr : 0;
    order = [first, ...order.filter((i) => i !== first)];
    const signs = item && item.signs;
    const pts = [];
    for (let i = 0; i < times.length; i++) {
      if (b.stopped) return null;
      const frame = await grabAt(cam, times[i]);
      for (let tries = 0; tries < 2; tries++) {
        const vi = order[tries];
        let text = '';
        for (const bd of bands) text += ` ${(await ocrRegion(w, frame, bd, OCR_VARIANTS[vi], false)).text}`;
        if (b.stopped) return null;
        const c = parseCoordsText(text, signs);
        if (c) {
          pts.push({ t: times[i] + cam.offset, lat: c.lat, lon: c.lon, latL: c.latL, lonL: c.lonL, speed: parseSpeedText(text), clock: parseClockText(text) });
          if (tries) order = [vi, ...order.filter((x) => x !== vi)];
          break;
        }
      }
      busySet(`Leyendo coordenadas en tu computador ${i + 1}/${times.length}…`, (i + 1) / times.length);
    }
    const clean = cleanTrack(pts);
    if (clean.length < 2) throw { code: 'local', message: 'No se pudieron leer suficientes coordenadas. Revisa en la pestaña Recorrido que el recuadro contenga el texto completo.' };
    setTrack(clean, { source: 'video', cam: cam.label, read: times.length, ok: clean.length });
    return { ok: clean.length, read: times.length, failed: 0 };
  }

  // Gemini (Google) con la clave gratuita del usuario, con la misma forma que `sample`.
  const GEMINI = 'https://generativelanguage.googleapis.com/v1beta';
  const GEM_KEY = 'cofcam:gemini';
  ...
  async function gemFetch(path, key, init) {
    let res;
    try {
      res = await fetch(`${GEMINI}${path}`, { ...init, headers: { 'Content-Type': 'application/json', 'x-goog-api-key': key, ...((init && init.headers) || {}) } });
    } catch (e) {
    ...
    const code = res.status === 429 ? 'rate_limited' : res.status === 404 ? 'bad_model' : res.status === 400 && /api key/i.test(msg) ? 'bad_key' : res.status === 403 || res.status === 401 ? 'bad_key' : 'upstream_error';
    throw { code, message: msg, status: res.status, retryAfter: Number(res.headers.get('retry-after')) || 0 };
  }
  function pickModels(list) {
    const names = list
      .filter((m) => (m.supportedGenerationMethods || []).includes('generateContent'))
      .map((m) => String(m.name || '').replace(/^models\//, ''))
      .filter((n) => /^gemini-/.test(n) && /flash/.test(n) && !/(tts|image|live|audio|embed|thinking|computer|robotics|native)/.test(n));
    const ver = (n) => parseFloat((n.match(/gemini-(\d+(?:\.\d+)?)/) || [])[1] || '0');
    const stable = (n) => (/(preview|exp|latest)/.test(n) || /-\d{3,}$/.test(n) ? 0 : 1);
    const rank = (arr) => arr.sort((a, b) => stable(b) - stable(a) || ver(b) - ver(a) || a.length - b.length);
    const lite = rank(names.filter((n) => /lite/.test(n)));
    const flash = rank(names.filter((n) => !/lite/.test(n)));
    return { quick: lite[0] || flash[0] || null, main: flash[0] || lite[0] || null };
  }
  async function discoverGemini(key) {
    try {
      const data = await gemFetch('/models?pageSize=200', key, { method: 'GET' });
      // Google retira modelos para usuarios nuevos; los alias -latest siempre apuntan al vigente.
      const names = (data.models || []).map((m) => String(m.name || '').replace(/^models\//, ''));
      if (names.includes('gemini-flash-lite-latest') && names.includes('gemini-flash-latest')) return { quick: 'gemini-flash-lite-latest', main: 'gemini-flash-latest' };
      const picked = pickModels(data.models || []);
      if (picked.quick && picked.main) return picked;
    } catch (e) {
      if (e && e.code === 'bad_key') throw e;
    }
    return { quick: 'gemini-flash-lite-latest', main: 'gemini-flash-latest' };
  }
  function makeGemini(cfg) {
    async function call(input, opts, wantJson) {
      const o = opts || {};
      const text = typeof input === 'string' ? input : input.map((t) => `${t.role === 'user' ? 'Operador' : 'Analista'}: ${t.content}`).join('\n\n');
      const parts = [{ text }];
      for (const img of o.images ? [].concat(Array.from(o.images)) : []) parts.push({ inlineData: { mimeType: 'image/jpeg', data: await blobToB64(img) } });
      const model = o.modelTier === 'quick' ? cfg.quick : cfg.main;
      const body = JSON.stringify({
        contents: [{ role: 'user', parts }],
        generationConfig: { temperature: 0.2, maxOutputTokens: 8192, ...(wantJson ? { responseMimeType: 'application/json' } : {}) },
      });
      let data;
      for (let attempt = 0; ; attempt++) {
        try {
          data = await gemFetch(`/models/${model}:generateContent`, cfg.key, { method: 'POST', body, signal: o.signal });
          break;
        } catch (e) {
          if (e && e.code === 'rate_limited' && attempt === 0) {
            busySet('Gemini pidió esperar un momento por su límite gratuito…', null);
            await waitMs(clamp((e.retryAfter || 20) * 1000, 5000, 60000), o.signal);
            continue;
          }
          throw e;
        }
      ...
      if (!out) throw { code: cand && /SAFETY|PROHIBITED|BLOCK/.test(cand.finishReason || '') ? 'refused' : 'empty_completion', message: 'sin texto' };
      return { text: out, truncated: !!cand && cand.finishReason === 'MAX_TOKENS' };
    }
    const sample = async (input, opts) => {
      const r = await call(input, opts, false);
      if (opts && opts.onText) opts.onText({ text: r.text, delta: r.text });
      return { ...r, modelTierApplied: (opts && opts.modelTier) || 'default' };
    };
    sample.json = async (input, opts) => {
      ...
      try {
        ...
        throw { code: 'invalid_json', message: 'La respuesta no era JSON', text: r.text };
      }
    };
    sample.limits = async () => ({ maxPromptBytes: 500000, images: { maxCount: 16 } });
    return sample;
  }
  function setGemini(cfg) {
    if (cfg && cfg.key) {
      S.gem = cfg;
      S.sample = makeGemini(cfg);
      S.limits = { maxPromptBytes: 500000, images: { maxCount: 16 } };
      S.aiOff = null;
      setPill('#aiPill', 'on', 'IA: Gemini lista');
      $('#gemStatus').textContent = `Clave guardada en este computador · modelos: ${cfg.quick} y ${cfg.main}.`;
    } else {
      S.gem = null;
      S.sample = null;
      S.limits = null;
      setPill('#aiPill', 'wait', 'IA: falta la clave de Gemini');
      $('#gemStatus').textContent = '';
    }
    $('#gemForget').hidden = !S.gem;
    renderPrep();
    updateEstimates();
    refreshButtons();
  }
  async function saveGeminiKey(key) {
    const k = key.trim();
    if (!/^[\w.-]{20,}$/.test(k)) return toast('Esa clave no parece válida. Cópiala completa desde aistudio.google.com/apikey.', 'err');
    const st = $('#gemStatus');
    st.textContent = 'Probando la clave…';
    try {
      const models = await discoverGemini(k);
      const cfg = { key: k, ...models };
      store.set(GEM_KEY, cfg);
      $('#gemKey').value = '';
      setGemini(cfg);
      toast('Clave de Gemini guardada. Ya puedes preparar el video.', 'ok');
      S.autoTried = false;
      maybeAutoRun();
    } catch (e) {
      st.textContent = '';
      toast(e && e.code === 'bad_key' ? 'Google rechazó la clave. Revisa que la copiaste completa y que esté activa.' : 'No se pudo comprobar la clave. Revisa tu conexión y vuelve a intentarlo.', 'err');
    }
  }
  ...
  function initLocal() {
    S.mcp = null;
    S.downloads = LOCAL_DOWNLOADS;
    setPill('#mapPill', 'on', 'Mapa: Esri y OpenStreetMap');
    setMapNote('Cambia entre calles y satélite abajo a la izquierda. Sin costo.');
    $('#gemForm').hidden = false;
    $('#privacyNote').textContent =
      'Las coordenadas se leen en tu computador. Para describir escenas se envían a Gemini (Google) solo los fotogramas de cada consulta; con la capa gratuita, Google puede usar esos datos para mejorar sus productos. El video completo no sale de tu equipo.';
    setGemini(store.get(GEM_KEY, null));
  }
  async function addressLocal(p) {
    const url = `https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${p.lat.toFixed(6)}&lon=${p.lon.toFixed(6)}&accept-language=es&zoom=18`;
    const res = await fetch(url, { headers: { Accept: 'application/json' } });
    if (!res.ok) throw new Error(String(res.status));
    const j = await res.json();
    const a = j.address || {};
    const street = [a.road, a.house_number].filter(Boolean).join(' ');
    return [street, a.suburb || a.neighbourhood, a.city || a.town || a.village].filter(Boolean).join(', ') || j.display_name || null;
  }
        ...
        el(
          ...
          el(
            ...
            el(
              ...
              el('li', { text: 'Todo se prepara solo. Puedes seguirlo en el panel Preparación del video.' }),
              el('li', { text: 'La página lee las coordenadas, arma el recorrido y crea un índice de lo que pasa. Luego busca arriba lo que necesites.' }),
            ),
            el(
              'div',
              { class: 'btn-row' },
  ...
  function cleanTrack(input) {
    ...
    for (const key of ['lat', 'lon']) {
      // Mandan las lecturas donde se vio la letra del hemisferio, si las hay.
      const lettered = pts.filter((p) => p[`${key}L`]);
      const voters = lettered.length ? lettered : pts;
      const sign = voters.filter((p) => p[key] < 0).length >= voters.length / 2 ? -1 : 1;
    ...
    // La hora del equipo avanza al mismo ritmo que el video: se descartan las mal leídas.
    const offs = pts.filter((p) => p.clock != null).map((p) => p.clock - p.t * 1000);
    if (offs.length >= 3) {
      const mo = median(offs);
      for (const p of pts) if (p.clock != null && Math.abs(p.clock - p.t * 1000 - mo) > 3000) p.clock = null;
    }
    const out = [];
    for (const p of pts) {
  ...
  function initMap() {
    ...
    Mp.map = L.map('map', { zoomControl: true, attributionControl: !IN_CLAUDE, zoomSnap: 0.5, minZoom: 2, maxZoom: 19 }).setView([-20, -65], 3);
    if (!IN_CLAUDE) {
      // Esri no exige clave ni cabecera de origen, así que funciona abriendo el archivo con doble clic.
      const esri = (svc) => `https://server.arcgisonline.com/ArcGIS/rest/services/${svc}/MapServer/tile/{z}/{y}/{x}`;
      const bases = {
        Calles: L.tileLayer(esri('World_Street_Map'), { maxZoom: 19, maxNativeZoom: 19, attribution: 'Mapa © Esri, HERE, Garmin, © OpenStreetMap' }),
        Satélite: L.tileLayer(esri('World_Imagery'), { maxZoom: 19, maxNativeZoom: 19, attribution: 'Imágenes © Esri, Maxar, Earthstar Geographics' }),
        OpenStreetMap: L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom: 19, attribution: '© OpenStreetMap' }),
      };
      const saved = store.get('cofcam:base', 'Calles');
      (bases[saved] || bases.Calles).addTo(Mp.map);
      L.control.layers(bases, {}, { collapsed: false, position: 'topleft' }).addTo(Mp.map);
      Mp.map.on('baselayerchange', (e) => store.set('cofcam:base', e.name));
    }
  ...
  async function loadBase(ptsOverride) {
    ...
    if (!Mp.map || !pts.length || !S.mcp || S.mapOff) {
      if (IN_CLAUDE && S.mcp === null && pts.length) setMapNote('Sin mapa base en esta vista: la ruta se dibuja sobre una cuadrícula.');
      return;
    }
  ...
  async function whereIsBus() {
    ...
    if (!text && !IN_CLAUDE) {
      try {
        text = await addressLocal(p);
        if (text) S.addrCache.set(key, text);
      } catch {
        return toast('No se pudo consultar la dirección ahora. Intenta de nuevo en unos segundos.', 'err');
      }
    }
    if (!text && !S.mcp) return toast('La búsqueda de direcciones usa el conector de TomTom y solo funciona dentro de claude.ai.', 'err');
    if (!text) {
      const btn = $('#btnAddr');
      btn.disabled = true;
  ...
  async function analyzeMoment() {
    ...
    try {
      ...
      busySet(`${AI_NAME} está analizando las cámaras…`, null);
      const prompt = [
        RULES,
        `Contexto:\n${contextText()}`,
  ...
  async function coreIndex(b) {
    ...
    for (let i = 0; i < times.length; i += per) {
      ...
      busySet(`${AI_NAME} está describiendo los momentos ${i + 1} a ${i + used.length} de ${times.length}…`, (i + used.length) / times.length);
      const prompt = [
        RULES,
        `Contexto:\n${contextText()}`,
        focusText($('#extraInstr').value.trim()),
  ...
  function renderPrep() {
    ...
    const steps = [
      ...
      },
      {
        ...
        s: run === 2 ? 'run' : S.index.length ? 'ok' : !IN_CLAUDE && !S.sample ? 'skip' : 'pend',
        det: S.index.length ? `${S.index.length} momento${S.index.length === 1 ? '' : 's'}` : !IN_CLAUDE && !S.sample ? 'falta clave de Gemini' : '',
      },
    ];
    box.replaceChildren(
      ...steps.map((st, i) =>
    ...
    const warn = $('#prepWarn');
    const msg = !IN_CLAUDE ? S.aiOff || '' : S.sample === null ? 'El análisis con Claude solo funciona con la página abierta dentro de claude.ai.' : S.aiOff || (imagesOff() ? NO_IMAGES : '');
    warn.hidden = !msg;
    warn.textContent = msg;
    const all = osdDone && (S.index.length || (!IN_CLAUDE && !S.sample)) && (S.track.length || !(item && item.has));
    ...
    if (!S.cams.length) est.textContent = 'Carga los videos para calcular el consumo.';
      ...
      const n = indexCost().calls;
      est.textContent = S.sample ? `Coordenadas: gratis en tu computador · índice: ${n} consulta${n === 1 ? '' : 's'} a Gemini` : 'Coordenadas: gratis en tu computador';
  ...
  async function runPipeline() {
    ...
    try {
      ...
      S.prepRun = 2;
      renderPrep();
      if (S.sample && !imagesOff()) {
        if (!S.index.length || redo) await coreIndex(b);
        if (b.stopped) return;
        toast('Video preparado. Escribe arriba lo que quieras buscar.', 'ok');
      } else if (!IN_CLAUDE) {
        toast(S.track.length ? 'Recorrido listo. Para que la IA describa las cámaras, pega tu clave gratuita de Gemini.' : 'Para que la IA describa las cámaras, pega tu clave gratuita de Gemini.', S.track.length ? 'ok' : 'err');
      }
      $('#q').focus();
    } catch (e) {
      onAiError(e);
  ...
  function maybeAutoRun() {
    ...
    if (IN_CLAUDE) {
      ...
      const item = osdItem(S.osd.camKey);
      const ocrDone = S.osd.items.length && (S.track.length || !(item && item.has));
      if (ocrDone && (S.index.length || !S.sample)) return;
    }
    S.autoTried = true;
    runPipeline();
  }
  ...
  async function search(q) {
    ...
    if (!S.index.length && !S.analyses.length && !S.track.length) {
      toast(S.cams.length ? 'Primero pulsa “Preparar video” para que la IA tenga de dónde buscar.' : 'Primero carga los videos de las cámaras.', 'err');
      return;
    }
  ...
  async function coreDetect(b) {
    ...
    busySet(`${AI_NAME} está leyendo el texto en pantalla…`, null);
    const prompt = [
  ...
  async function detectOSD() {
    ...
    try {
      const chosen = await detectAny(b);
      if (b.stopped) return;
      renderAll();
  ...
  function ocrPlan(cam) {
    ...
    const every = v === 'auto' ? (IN_CLAUDE ? Math.max(5, Math.ceil(dur / 150)) : Math.max(2, Math.ceil(dur / 300))) : Number(v);
    const times = [];
  ...
  function ocrEstimate() {
    ...
    const c = ocrCost(cam);
    out.textContent = IN_CLAUDE ? `${c.n} lecturas (cada ${c.every} s) · ${c.calls} consulta${c.calls === 1 ? '' : 's'} · ≈ ${kTok(c.tokens)}` : `${c.n} lecturas (cada ${c.every} s) · gratis, en tu computador`;
  }
  function updateEstimates() {
    ...
    box.textContent = IN_CLAUDE
      ? `“¿Qué pasa aquí?” gasta ≈ ${kTok(moment)} con el modelo ${TIERS[S.tier].toLowerCase()}. Buscar arriba gasta ≈ ${kTok(3500)} (solo texto).`
      : `Cada “¿Qué pasa aquí?” y cada búsqueda es 1 consulta a Gemini (gratis con tu clave, con límite diario).`;
    renderPrep();
  }
  ...
  async function coreExtract(b) {
    ...
    for (let gi = 0; gi < groups.length; gi++) {
      if (b.stopped) return null;
      busySet(`${AI_NAME} está leyendo las coordenadas ${gi + 1}/${groups.length}…`, 0.5 + (gi / groups.length) * 0.5);
      const g = groups[gi];
  ...
  // Fuera de claude.ai se lee primero en el computador (gratis) y Gemini entra solo si no alcanza.
  async function detectAny(b) {
    if (IN_CLAUDE) return coreDetect(b);
    const found = await detectLocal(b);
    if (found || b.stopped || !S.sample) return found;
    busySet('El lector local no encontró coordenadas; se las pido a Gemini…', null);
    try {
      return await coreDetect(b);
    } catch (e) {
      if (e && e.code === 'cancelled') throw e;
      return null;
    }
  }
  async function extractAny(b) {
    if (IN_CLAUDE) return coreExtract(b);
    let local = null;
    let err = null;
    try {
      local = await extractLocal(b);
    } catch (e) {
      if (!(e && e.code === 'local')) throw e;
      err = e;
    }
    if (b.stopped) return null;
    if (!S.sample || (local && local.ok >= local.read * 0.6)) {
      if (err) throw err;
      return local;
    }
    busySet('Varias lecturas salieron dudosas; Gemini revisa las coordenadas…', null);
    try {
      return await coreExtract(b);
    } catch (e) {
      if (e && e.code === 'cancelled') throw e;
  ...
  async function writeReport() {
    ...
    const out = $('#report');
    const b = busyStart(`${AI_NAME} está redactando el informe…`);
  ...
  function renderSession() {
    ...
    if (IN_CLAUDE && S.usage) box.append(chip('Uso aprox.', kTok(S.usage)));
    if (!IN_CLAUDE && S.aiCalls) box.append(chip('Consultas IA', String(S.aiCalls)));
  }
  ...
  function bind() {
    ...
    $('#gemForm').addEventListener('submit', (e) => {
      e.preventDefault();
      saveGeminiKey($('#gemKey').value);
    });
    $('#gemForget').addEventListener('click', () => {
      store.set(GEM_KEY, null);
      setGemini(null);
      toast('Clave de Gemini olvidada en este computador.');
    });
